...
class Generator(nn.Module):
    ...
    def forward(self, raw, exp, mode):
        ...
        if mode == 'train':
            ...
            raw_y2 = self.upsample2(res_all)
            raw_y2 = torch.cat([raw_y2, raw3], dim=1)
            raw_y2 = self.dec2(raw_y2)

            raw_y3 = self.upsample3(raw_y2)
            raw_y3 = torch.cat([raw_y3, raw2], dim=1)
            ...
            exp_y2 = self.upsample2(res_all2)
            exp_y2 = torch.cat([exp_y2, exp3], dim=1)
            exp_y2 = self.dec2(exp_y2)

            exp_y3 = self.upsample3(exp_y2)
            exp_y3 = torch.cat([exp_y3, exp2], dim=1)
