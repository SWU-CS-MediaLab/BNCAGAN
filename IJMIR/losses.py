...
class PerceptualLoss(nn.Module):
    ...
    def __call__(self, x, y):
        ...
        # 前四个权重很小，第五个权重最大
        # 权重为[1.0/64, 1.0/64, 1.0/32, 1.0/32, 1.0/1]
...
class VGG19_relu(torch.nn.Module):
    def __init__(self):
        ...
        cnn.load_state_dict(torch.load(os.path.join('BNCAGAN/models/', 'vgg19-dcbb9e9d.pth')))
...
class GANLoss(nn.Module):
    ...
    def loss(self, real_preds, fake_preds, target_is_real, for_real=None, for_fake=None, for_discriminator=True):
        if self.gan_mode == 'original': # cross entropy loss
            ...
            if for_discriminator:
                ## difference between real and fake（一张真图与其他假图的差别）
                r_f_diff = real_preds - torch.mean(fake_preds)
                ## difference between fake and real（一张假图与其他真图的差别）
